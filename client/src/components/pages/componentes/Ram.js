...
const Ram = () => {
  ...
  // Función para obtener las memorias RAM de la API
  useEffect(() => {
    const fetchRam = async () => {
      try {
        ...
        // Filtrar memorias RAM usando el campo 'type'
        const ram = response.data.filter(product => {
          // Primero excluimos explícitamente las tarjetas gráficas
          if (product.type === 'GPU' || product.type === 'Graphics Card' ||
              (product.name && (
                product.name.toLowerCase().includes('tarjeta gráfica') ||
                product.name.toLowerCase().includes('tarjeta grafica') ||
                product.name.toLowerCase().includes('geforce') ||
                product.name.toLowerCase().includes('radeon') ||
                product.name.toLowerCase().includes('amd rx') ||
                product.name.toLowerCase().includes('nvidia')
              ))) {
            return false;
          }
          
          // Luego verificamos si es una memoria RAM
          return product.type === 'RAM' || product.type === 'Memory' || 
                 (product.name && (
                   product.name.toLowerCase().includes('memoria ram') ||
                   product.name.toLowerCase().includes('módulo ram') ||
                   product.name.toLowerCase().includes('modulo ram') ||
                   (product.name.toLowerCase().includes('ram') && !product.name.toLowerCase().includes('tarjeta')) ||
                   (product.name.toLowerCase().includes('memoria') && !product.name.toLowerCase().includes('tarjeta')) ||
                   product.name.toLowerCase().includes('ddr')
                 ));
        });
        
        console.log('Memorias RAM encontradas:', ram);
        setProducts(ram);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar las memorias RAM:', err);
        setError('Error al cargar las memorias RAM');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Mejora el rendimiento de tu ordenador con nuestras memorias RAM
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=RAM'}
                  alt={product.name}
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <CardMedia
                    component="img"
                    image={selectedProduct.image || 'https://via.placeholder.com/300x200?text=RAM'}
