...
const PlacasBase = () => {
  ...
  // Función para obtener las placas base de la API
  useEffect(() => {
    const fetchPlacasBase = async () => {
      try {
        setLoading(true);
        ...
        // Filtrar placas base usando el campo 'type'
        const placasBase = response.data.filter(product => {
          return product.type === 'Motherboard' || 
                 (product.name && product.name.toLowerCase().includes('placa base'));
        });
        
        console.log('Placas base encontradas:', placasBase);
        setProducts(placasBase);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar las placas base:', err);
        setError('Error al cargar las placas base');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Descubre nuestra selección de placas base para tu ordenador
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=Motherboard'}
                  alt={product.name}
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <CardMedia
                    component="img"
                    image={selectedProduct.image || 'https://via.placeholder.com/300x200?text=Motherboard'}
