...
const Refrigeracion = () => {
  ...
  // Función para obtener los sistemas de refrigeración de la API
  useEffect(() => {
    const fetchRefrigeracion = async () => {
      try {
        setLoading(true);
        ...
        // Filtrar productos de refrigeración
        const refrigeracion = response.data.filter(product => {
          // Excluir explícitamente otros componentes
          if (product.type === 'GPU' || product.type === 'CPU' || 
              product.type === 'RAM' || product.type === 'Storage' ||
              product.type === 'Motherboard' || product.type === 'PSU') {
            return false;
          }
          
          // Verificar si es un producto de refrigeración
          return product.type === 'Cooling' || product.type === 'Cooler' || 
                 (product.name && (
                   product.name.toLowerCase().includes('refrigera') ||
                   product.name.toLowerCase().includes('ventilador') ||
                   product.name.toLowerCase().includes('cooler') ||
                   product.name.toLowerCase().includes('disipador') ||
                   product.name.toLowerCase().includes('watercooling') ||
                   product.name.toLowerCase().includes('refrigeración líquida') ||
                   product.name.toLowerCase().includes('refrigeracion liquida')
                 ));
        });
        
        console.log('Productos de refrigeración encontrados:', refrigeracion);
        setProducts(refrigeracion);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar los productos de refrigeración:', err);
        setError('Error al cargar los productos de refrigeración');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Mantén tu equipo a temperatura óptima con nuestros sistemas de refrigeración
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=Cooling'}
                  alt={product.name}
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <CardMedia
                    component="img"
                    image={selectedProduct.image || 'https://via.placeholder.com/300x200?text=Cooling'}
