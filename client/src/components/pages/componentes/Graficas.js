...
const Graficas = () => {
  ...
  // Función para obtener las tarjetas gráficas de la API
  useEffect(() => {
    const fetchGraficas = async () => {
      try {
        ...
        // Filtrar tarjetas gráficas usando el campo 'type'
        const graficas = response.data.filter(product => {
          // Primero nos aseguramos de que NO sea explícitamente un producto de RAM
          if (product.type === 'RAM') {
            return false;
          }
          
          // Luego verificamos si es una tarjeta gráfica
          return product.type === 'GPU' || product.type === 'Graphics Card' || 
                 (product.name && (
                   ...
                   (product.name.toLowerCase().includes('gráfica') && !product.name.toLowerCase().includes('memoria')) ||
                   (product.name.toLowerCase().includes('grafica') && !product.name.toLowerCase().includes('memoria'))
                 ));
        });
        
        console.log('Tarjetas gráficas encontradas:', graficas);
        setProducts(graficas);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar las tarjetas gráficas:', err);
        setError('Error al cargar las tarjetas gráficas');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Explora nuestra selección de tarjetas gráficas de alto rendimiento
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=GPU'}
                  alt={product.name}
