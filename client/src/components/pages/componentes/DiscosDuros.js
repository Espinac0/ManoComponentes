...
const DiscosDuros = () => {
  ...
  // Función para obtener los discos duros de la API
  useEffect(() => {
    const fetchDiscosDuros = async () => {
      try {
        ...
        // Filtrar discos duros usando el campo 'type'
        const discosDuros = response.data.filter(product => {
          return product.type === 'Storage' || product.type === 'HDD' || product.type === 'SSD' || 
                 (product.name && (
                   product.name.toLowerCase().includes('disco') ||
        ...
        console.log('Discos duros encontrados:', discosDuros);
        setProducts(discosDuros);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar los discos duros:', err);
        setError('Error al cargar los discos duros');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Amplía el almacenamiento de tu ordenador con nuestra selección de discos duros
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=Storage'}
                  alt={product.name}
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <CardMedia
                    component="img"
                    image={selectedProduct.image || 'https://via.placeholder.com/300x200?text=Storage'}
