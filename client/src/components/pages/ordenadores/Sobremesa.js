...
const Sobremesa = () => {
  ...
  // Función para obtener los ordenadores de sobremesa de la API
  useEffect(() => {
    const fetchSobremesa = async () => {
      try {
        ...
        // Filtrar ordenadores de sobremesa
        const sobremesa = response.data.filter(product => {
          return product.type === 'Desktop' || product.type === 'Sobremesa' || 
                 (product.name && (
                   product.name.toLowerCase().includes('sobremesa') ||
                   product.name.toLowerCase().includes('desktop') ||
                   product.name.toLowerCase().includes('torre') ||
                   (product.name.toLowerCase().includes('pc') && !product.name.toLowerCase().includes('portátil') && !product.name.toLowerCase().includes('portatil') && !product.name.toLowerCase().includes('laptop'))
                 ));
        });
        
        console.log('Ordenadores de sobremesa encontrados:', sobremesa);
        setProducts(sobremesa);
        setLoading(false);
      } catch (err) {
        console.error('Error al cargar los ordenadores de sobremesa:', err);
        setError('Error al cargar los ordenadores de sobremesa');
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="h5" align="center" color="text.secondary" paragraph>
          Descubre nuestra selección de ordenadores de sobremesa con el mejor rendimiento
        </Typography>
      </Box>
            ...
            <Grid item key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                ...
                <CardMedia
                  component="img"
                  height="180"
                  image={product.image || 'https://via.placeholder.com/300x200?text=Desktop'}
                  alt={product.name}
            ...
            <DialogContent sx={{ pt: 3 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <CardMedia
                    component="img"
                    image={selectedProduct.image || 'https://via.placeholder.com/300x200?text=Desktop'}
